...
The Admin Center renders an interactive CentralStationCRM-style settings shell,
not only an audit table. Tenant admins get grouped navigation for `Meine
Daten`, `Meine Einstellungen`, `Accounteinstellungen`, and `Papierkorb`; the
right-hand content pane switches to the selected subpage. `Meine Daten` and
`Meine Einstellungen` include real Streamlit controls, validation, and
consistent `Speichern`/`Abbrechen` behavior. Supported v1 preferences are saved
in the current Streamlit session only, so they prove the UX flow without
claiming durable profile persistence.

Marketing and pure partner landing pages are intentionally ignored; operational
integration setup pages remain visible. Tenant PostgreSQL provisioning, API
keys, webhook secrets, integration credentials, billing/payment updates,
restore actions, account reset, exports, and permanent deletions stay
backend-governed and audited outside the Streamlit UI. Those pages show the
right controls and empty states but warn instead of fake-saving unsupported
actions.
