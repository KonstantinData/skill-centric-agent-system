...
example `code_review`, `data_export`, `document_summary`.
...
- `signal` — The condition being evaluated. Format: `type:value`, for example
  `task_type:code_review`, `input:diff`, `phrase:deploy`, `risk_level:critical`.
...
behaves as specified. At minimum, include one positive fixture (task where the
module should be selected) and one negative fixture (task where it should not).
