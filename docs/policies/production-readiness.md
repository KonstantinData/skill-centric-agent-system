...
| Operational telemetry | Retrieval, validation, cleanup, AI Gateway, queue depth, claim latency, run duration, tenant saturation, retry rate, DLQ rate, quota exhaustion, runtime failures, and policy denials have observable signals and runbook-backed diagnostics. |
